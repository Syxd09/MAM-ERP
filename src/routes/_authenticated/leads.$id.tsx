    ...
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={!!lead.customer_id || convertToCustomer.isPending}
          >
            <CheckCircle2 className="size-4 mr-1" />{" "}
            {lead.customer_id ? "Converted" : "Convert to Customer"}
          </Button>
          <Link to="/quotations/new" search={{ lead: id } as any}>
            <Button variant="outline">
              <FileText className="size-4 mr-1" /> Quotation
            </Button>
          </Link>
          <Button
            variant="ghost"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="panel p-5 lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Field
              label="Source"
              value={LEAD_SOURCE_LABELS[lead.source as keyof typeof LEAD_SOURCE_LABELS]}
            />
            ...
            <div className="md:col-span-2">
              <Field label="Requirement" value={lead.requirement} pre />
            </div>
            <div className="md:col-span-2">
              <Field label="Notes" value={lead.notes} pre />
            </div>
          </div>
        </div>
        ...
        <div className="panel p-5 space-y-3">
          ...
          <div>
            ...
            <div className="mt-2">
              <span
                className={`text-[10px] uppercase tracking-widest px-2 py-1 rounded border ${STATUS_TONE[lead.status as LeadStatus]}`}
              >
      ...
      <div className="panel p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <NewFollowUpDialog
            leadId={id}
            onSaved={() => qc.invalidateQueries({ queryKey: ["followups", id] })}
          />
        </div>
        {followUps.length === 0 ? (
          ...
          <div className="space-y-2">
            {followUps.map((f: any) => (
              <div
                key={f.id}
                className={`flex items-center justify-between p-3 rounded-md border ${f.completed ? "bg-success/5 border-success/20 opacity-60" : "bg-background/40 border-border/50"}`}
              >
...
function Field({ label, value, pre }: { label: string; value: any; pre?: boolean }) {
  return (
    <div>
      ...
      <div className={`text-sm mt-1 ${pre ? "whitespace-pre-wrap" : ""}`}>
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("follow_ups")
        .insert({ lead_id: leadId, due_date: date, notes, created_by: user?.id });
      if (error) throw error;
    },
    onSuccess: () => {
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="e.g. Call to confirm material spec"
            />
          </div>
        </div>
        <DialogFooter>
